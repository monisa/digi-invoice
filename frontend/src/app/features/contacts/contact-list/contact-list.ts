...
export class ContactList {
  ...
  load(): void {
    ...
    this.service
      ...
      .subscribe({
        ...
        error: (err) => {
          this.loading.set(false);
          this.snack.open(extractApiError(err, 'Failed to load contacts'), 'Dismiss', { duration: 5000 });
        },
      });
  }
  ...
  openCreate(): void {
    this.dialog
      .open(ContactForm, { data: {} })
      .afterClosed()
      .subscribe((result?: Contact) => {
        if (result) {
          this.snack.open('Contact created', undefined, { duration: 2500 });
          this.load();
        }
      });
  }
  ...
  openEdit(contact: Contact): void {
    this.dialog
      .open(ContactForm, { data: { contact } })
      .afterClosed()
      .subscribe((result?: Contact) => {
        if (result) {
          this.snack.open('Contact updated', undefined, { duration: 2500 });
          this.load();
        }
      });
  }
  ...
  confirmDelete(contact: Contact): void {
    this.dialog
      ...
      .subscribe((confirmed: boolean) => {
        if (!confirmed) return;
        this.service.remove(contact.id).subscribe({
          next: () => {
            this.snack.open('Contact deleted', undefined, { duration: 2500 });
            this.load();
          },
          error: (err) =>
